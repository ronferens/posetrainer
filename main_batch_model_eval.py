...
def main(cfg) -> None:
    ...
    batch_eval_results = []
    logging.info("Found {} models for evaluation".format(len(models_to_check)))

    for idx, ckpt in enumerate(models_to_check):
        if 'last' in str(ckpt):
        ...
        logging.info("[{}/{}]) Initializing from checkpoint: {}".format((idx + 1), len(models_to_check), model_name))
